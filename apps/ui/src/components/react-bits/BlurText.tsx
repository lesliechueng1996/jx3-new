...
const BlurText: React.FC<BlurTextProps> = ({
  ...
  const keyedElements = useMemo(() => {
    ...
    return parts.map((segment, position) => {
      const occurrence = seen.get(segment) ?? 0;
      seen.set(segment, occurrence + 1);
      return { segment, key: `${segment}-${occurrence}`, position };
    });
  }, [animateBy, text]);
  ...
  return (
    <span ref={ref} className={`blur-text ${className} flex flex-wrap`}>
      {keyedElements.map((item) => {
        const animateKeyframes = buildKeyframes(fromSnapshot, toSnapshots);

        const spanTransition: Transition = {
          duration: totalDuration,
          times,
          delay: (item.position * delay) / 1000,
          ease: easing,
        };
        ...
        return (
          <motion.span
            ...
            onAnimationComplete={
              item.position === keyedElements.length - 1
                ? onAnimationComplete
                : undefined
            }
            style={{
